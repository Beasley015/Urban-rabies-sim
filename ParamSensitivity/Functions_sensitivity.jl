...
function initialize_disease(;dat, nstart)
    ...
    new_diseases = ifelse(length(unvax) > nstart, sample(unvax, nstart, replace = false, ordered = true), unvax)
...
# Mortality function
function dont_fear_the_reaper(;dat, home, k, amort, jmort)
    # random mortality
    ...
    too_many_guys = findall(length.(indices) .> k) 
    ...
    # Decide who dies
    dead_adults = rand(Bernoulli(amort), length(crowded_adults))
    dead_juvies = rand(Bernoulli(jmort), length(crowded_juvies))
